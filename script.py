...
# Set to your current working directory
DIRECTORY = "/home/dadi/Desktop/Labs_6th sem/ComputationalBiophysics"

def run_git_command(command, cwd=None):
    """Run a Git command in the specified directory"""
    return subprocess.run(command, capture_output=True, text=True, cwd=cwd or DIRECTORY)
IGNORED_PATTERNS = ["env/", ".venv/", ".ipynb_checkpoints/", "script.py"]

def is_ignored(filename):
    return any(part in filename for part in IGNORED_PATTERNS)

def get_untracked_files():
    """Get a list of untracked (new) files, excluding virtualenvs, checkpoints, and script.py"""
    result = run_git_command(["git", "ls-files", "--others", "--exclude-standard"])
    files = [f for f in result.stdout.splitlines() if not is_ignored(f)]
    print(f"Untracked files: {files}")
    return files

def get_modified_files():
    """Get a list of modified files, excluding virtualenvs, checkpoints, and script.py"""
    result = run_git_command(["git", "status", "--porcelain"])
    files = [line.split()[-1] for line in result.stdout.splitlines()
             if line.startswith(" M ") and not is_ignored(line.split()[-1])]
    print(f"Modified files: {files}")
    return files

def get_deleted_files():
    """Get a list of deleted files, excluding virtualenvs, checkpoints, and script.py"""
    result = run_git_command(["git", "status", "--porcelain"])
    files = [line.split()[-1] for line in result.stdout.splitlines()
             if line.startswith(" D ") and not is_ignored(line.split()[-1])]
